...
public:
  ...
  Robot(){//Constructor w/o parameters, assign member variables to default(assignes as constants)                                                    
    m_id = DEFAULT_ID;
    m_type = DEFAULT_TYPE;
...
private:
  ...
  Robot* insert(const Robot& robot,Robot*& aNode);//Helper,recursivley inserts                                                                       
  void clear(Robot* aBot);//Helper,recursivley deallocates tree                                                                                      
  Robot* remove(Robot* aBot,int id);//Helper,recursivly iterates thru tree to delete                                                                 
  Robot* findMin(Robot* aBot);//Helper, recursiviley finds min value in tree                                                                         
  Robot* leftRotation(Robot* aBot);//Helper, recusivly rotates tree to left                                                                          
  Robot* rightRotation(Robot* aBot);//Helper, recursively rotates tree to right                                                                      
  void listRobots(Robot* aBot) const;//Helper, recursively lists robots state, type in inOrder traversal                                             
  Robot* findBot(int id, Robot* aBot) const;//Helper, recursively traverses tree to find bot, returns true if found                                  
  void removeDead(Robot* aBot);//Helper,recursivley iterates thru tree finds DEAD bots and removes them                                              
